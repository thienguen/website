  ...
  return (
    ...
    >
      <div className="flex h-full flex-col justify-center p-5 ">
        <h1 className="text-[14px] font-bold text-slate-600 dark:font-bold dark:text-white sm:text-lg">
          {props.question}
        </h1>
        {props.customAnswer ? (
          props.customAnswer
        ) : (
          <p className="mt-2 font-metropolis text-gray-500 dark:text-gray-400">{props.answer}</p>
        )}
      </div>
    </div>
  )
